...
import com.loftechs.sample.call.create.CreateItemType
...
abstract class AbstractCreatePresenter : CreateContract.Presenter<CreateContract.View> {
        ...
        val subscribe = UserManager.getUserStatusWithSemiUIDs(listOf(inputString))
            ...
            .flatMap {
                if (it.userID.isNullOrEmpty()) {
                    ...
                    ProfileInfoManager.getProfileInfoByUserID(mReceiverID, it.userID, it.semiUID)
                }
            }
            .flatMap {
                ProfileInfoManager.updateProfileInfo(it)
    ...
    private fun queryData() {
        itemList = ArrayList()
        itemList.addAll(ProfileInfoManager.getLocalAllProfileInfo())
        mView?.refreshList(itemList)
    }
}