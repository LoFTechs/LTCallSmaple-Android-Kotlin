...
class CallListPresenter : CallListContract.Presenter<CallListContract.View> {
    ...
    override fun onBindViewHolder(view: CallListAdapter.IItemView, position: Int) {
        ...
        val subscribe =
            ProfileInfoManager.getProfileInfoByUserID(
                mReceiverID,
                getCallUserID(callLogData),
                getCallName(callLogData)
            )
                .doOnNext {
    ...
    private fun getCallName(callLogData: CallLogData): String {
        if (isSelfID(getCallUserID(callLogData))) {
            return callLogData.callee.semiUID ?: callLogData.callee.phoneNo
            ?: callLogData.callee.userID
        }
        return callLogData.caller.semiUID ?: callLogData.caller.phoneNo ?: callLogData.caller.userID
    }
